  ...
  (let ((recipe (intern path)))
    (straight--with-plist (straight--convert-recipe recipe) (host repo)
      ...
      (let ((url (if-let ((domain (car (alist-get host straight-hosts))))
        ...
        (cond
         ((eq backend 'html) (format "<a target=\"_blank\" href=\"%s\">%s</a>" url description))
          ((eq backend 'latex) (format "\\href{%s}{%s}" url description))
          ((eq backend 'texinfo) (format "@uref{%s,%s}" url description))
          ((eq backend 'ascii) (format "%s (%s)" description url))
          ((eq backend 'md) (format "[%s](%s)" description url))
          (t url))))))
